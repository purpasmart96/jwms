...
static void WriteJWMTIcons(JWMTray *tray, DArray *entries)
{
    FILE *fp;
    char path[512];
    ...
    fprintf(fp, "<JWM>\n");
    fprintf(fp, "<IconPath>/usr/share/icons/Papirus/16x16/actions/</IconPath>\n");
    fprintf(fp, "</JWM>");
}
...
static void GenJWMRootMenu(JWMRootMenu *root_menu, DArray *entries, HashMap *icons)
{
    FILE *fp;
    char path[512];
...
int WriteJWMConfig(DArray *entries, HashMap *icons)
{
    cfg_opt_t opts[] =
    {
        ...
        CFG_STR("global_bg_color_active", "100.111.100", CFGF_NONE),
        CFG_STR("global_bg_color_inactive", "100.111.100", CFGF_NONE),
    ...
    GenJWMRootMenu(root_menu, entries, icons);
